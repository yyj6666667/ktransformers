...
from kt_kernel.sft.lora import get_kt_rank_local_parameter_names, kt_adapt_peft_lora
...
def _write_non_expert_cache_manifest(cache, source):
    source_config_digest = _sha256(source / "config.json")
    source_index_digest = _sha256(source / "model.safetensors.index.json")
    source_fingerprint = artifacts_module._source_fingerprint(
        source_config_digest, source_index_digest
    )
    files = [cache / "model.safetensors.index.json", *sorted(cache.glob("*.safetensors"))]
    records = [
        {"name": path.name, "size": path.stat().st_size, "sha256": _sha256(path)}
        for path in files
    ]
    tensors = {"count": 1, "bytes": 8, "dtypes": {"BF16": 1}}
    _write_json(
        cache / KT_NON_EXPERT_MANIFEST_NAME,
        {
            ...
            "converter": {
                "name": "kt-kernel.prepare-non-expert-cache",
                "version": 2,
                "default_dtype": "BF16",
...
def _make_pretrained_artifacts(tmp_path, *, routed_schema_version=2):
    ...
    _write_non_expert_cache_manifest(cache, source)
...
def test_v2_manifest_resolves_a_strict_pretrained_load_plan(tmp_path):
...
def test_adaptation_is_idempotent_and_publishes_stable_parameter_names(monkeypatch):
    ...
    assert [name for name, _ in first.named_optimizer_parameters] == [
